...
def _to_class_predictions(y_pred, y_reference):
    """
    Convert model outputs to valid class labels when needed.
    """
...
def compare_models(results):
    ...
    def selection_score(model_name):
        ...
        return (
            ...
            result["val_recall"],
            MODEL_SELECTION_PRIORITY.get(model_name, 0),
        )

    best_model_name = max(results.keys(), key=selection_score)
    best_val_f1 = max(results.keys(), key=lambda x: results[x]["val_f1"])
    
    print(f"\nBest Model (validation score + advanced tie-break): {best_model_name}")
    print(f"Best Model (by F1-Score only): {best_val_f1}")
...
def save_best_model(best_model_name, best_results, feature_names, label_encoder, sample_metadata=None):
    """
    ...
    model_metadata = {
        ...
        "selection_rule": "validation accuracy, F1, precision, recall, then advanced model priority",
    }
    if sample_metadata:
        model_metadata.update(sample_metadata)
